...
class CartController extends AppController
{
    ...
    public $uses = array('Product', 'Address', 'Order', 'OrderLine');
    ...
    public function confirm()
    {
        if (!empty($this->data)) {
            if ($this->data['confirm']) {
                ...
                $price_total = $this->_getPriceTotal($items);
                ...
                foreach ($items as $item) {
                    $line = array('OrderLine' => $item);
                    $line['OrderLine']['order_id'] = $this->Order->id;

                    $this->OrderLine->create();
                    if (!$this->OrderLine->save($line)) {
                        $this->Session->setFlash('The ' . $line['OrderLine']['name'] . ' could not be added to the order. The order has been cancelled.');
                        $this->Order->delete();
                        $this->redirect();
                    }
                }
